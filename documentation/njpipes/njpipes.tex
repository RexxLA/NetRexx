...
\title{\fontspec{Bodoni URW Light}Pipelines Quick Start Guide}
...
\subsubsection{From the NetRexx Workspace (nrws) with direct execution}
...
\begin{figure}[h]
  ...
  \caption{Run in the NetRexx Workspace}
  \label{fig:runnrws}
\end{figure}
...
Admittedly, the example in the previous chapter could have been done
with Unix pipes or at least with incorporation of stream utilities
like awk or sed.


To get a good idea of what can be done with Pipelines for NetRexx, look at the tasktest
...
% implementation.
When a filter is not delvered in the standard set of stages, it is
very easy to make one yourself in the NetRexx language. The model for
...
In NetRexx, that would be about the same, but for some small changes
incurred by the object oriented model of NetRexx, which you don't have in Classic Rexx. Here peekto(), readto() and
...
\end{figure}\begin{figure}[h]
  ...
  \caption{bagvendt.nrx under NetRexx}
...
\item Pipelines on the mainframe is an interpreted language with
  components as the scanner and the dispatcher; the NetRexx
...
\chapter{How to use a pipe in a NetRexx program}
...
\chapter{TCP/IP Networking using Pipes for NetRexx}
...
pipeline is a custom stage that is written in this tcpexample.njp file. 

\chapter{Deadlocks}
Pipes for NetRexx and Java detects deadlocks -- also called
\emph{Pipeline stalls} -- and outputs information to allow you to fix the problem.  Consider the following session:
\begin{lstlisting}
...
\end{enumerate}
% \input{stagesChapter}
\includepdf[pages={1-12},nup=1x1,landscape=false]{stages.pdf}
\include{appendixa}
\backmatter
\listoffigures
\listoftables
