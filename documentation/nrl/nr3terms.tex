...
A \emph{term} in NetRexx is a syntactic unit which describes some
...
\subsubsection{Simple terms}\label{"id"}
...
\subsubsection{Compound terms}\label{"id"}
...
An implementation may optionally provide a mechanism that disallows this
parenthesis omission.
\subsection{Evaluation of terms}\label{"id"}
...
of type \textbf{Rexx}.
\item 
Because that string is of type \textbf{Rexx}, the \textbf{Rexx} class
...
This returns a new string of type \textbf{Rexx},
"\textbf{rabbit}" (the second word in the original string).
\item 
In the same way as before, the \textbf{pos} method of
the \textbf{Rexx} class is then invoked on the new string, with
argument "\textbf{b}".
 
This returns a new string of type \textbf{Rexx}, "\textbf{3}"
...
\subsubsection{Simple term evaluation}
...
their value is a string of type \textbf{Rexx}.
...
type \textbf{Rexx}, constructed from that literal.
\item 
If the stub is a numeric symbol, its value is the string, of
type \textbf{Rexx}, constructed from that literal (as though the
...
current class, which has type \textbf{Rexx}.  In this case, the
...
type \textbf{Rexx}, and the type of the result will
be \textbf{Rexx}.
...
property in a class listed in the \texttt{uses} phrase of the
\texttt{class} instruction.
...
In this case, the stub describes a  \emph{dimensioned (see page \pageref{refdimtype}) 
type}:ea..
\item The symbol naming the reference is the name of a primitive type or
is the short name of a known class, and there are one or more
expressions within the brackets.
In this case, the reference is to an  \emph{array (see page \pageref{refarray}) 
constructor}:ea.; the expressions within the brackets must
...
\subsubsection{Continuation evaluation}
...
(type \textbf{Rexx}) or a dimensioned array.  In either case, it is
...
type (or of type \textbf{Rexx} if there is no appropriate
type).
\end{itemize}
...
\subsubsection{Arrays in terms}\label{"id"}
