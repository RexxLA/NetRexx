\chapter{Appendix C - Translator Options}\label{optionstable}
...
\item[address]
This option defaults to \keyword{address}. When \keyword{noaddress} is specified, the use of the \keyword{address} instruction in a program will be flagged as an error. 
...
\index{option,implicituses}
\index{flag,implicituses}
...
\item[targetvm]
\index{option, targetvm}
\index{flag, targetvm}
\index{targetvm option}
