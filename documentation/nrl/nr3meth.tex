...
\index{DEFAULT,on METHOD instruction}
index
...
\index{Methods,default}
\item[default]

A \emph{default method} can only exist in an interface class, and like a \emph{static method} it actually
includes an implementation.
A \emph{default method} is called when a class implementing the interface class
does not provide the actual method. It eases post-factum updates to interface classes:
it allows adding a method without needing to update all classes implementing the interface class.
...
constant). A \emph{static method} can also be defined in an interface class.
