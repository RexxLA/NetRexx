\chapter{Structure and General Syntax}\label{"id"}
\index{Clauses,}
\index{Semicolons,}
\index{,}
 A NetRexx program is built up out of a series of \emph{clauses} that
...
\subsection{Blanks and White Space}\label{"id"}
\index{Blank,}
\index{White space,}
...
\subsection{Comments}\label{"id"}
\index{Comments,}
\index{Delimiters,for comments}
\index{,}
\index{,}
\index{,}
 Commentary is included in a NetRexx program by means of
...
\textbf{Note: }It is recommended that NetRexx programs start with a block comment
...
\item{nthe escape sequence represents a new-line (line feed) character}
\item{rthe escape sequence represents a return (carriage return) character}
\item{fthe escape sequence represents a form-feed character}
\item{"the escape sequence represents a double-quote character}
\item{'the escape sequence represents a single-quote character}
\item{\textbackslash the escape sequence represents a backslash character}
\item{-the escape sequence represents a "null" character}
(the character whose encoding equals zero), used to indicate
continuation in a \texttt{say} instruction
\item{0(zero) the escape sequence represents a "null" character}
(the character whose encoding equals zero); an alternative
to \textbf{\textbackslash -}
\item{xhhthe escape sequence represents a character whose encoding is}
...
\item{uhhhhthe escape sequence represents a character whose encoding is}
...
characters (a-f), in either lowercase or uppercase.
 \textbf{Examples:}
\begin{alltt}
'You shouldn\textbackslash 't'  /* Same as "You shouldn't" */
'\textbackslash x6d\textbackslash u0066\textbackslash x63'  /* In Unicode: 'mfc' */
'\textbackslash \textbackslash \textbackslash u005C'        /* In Unicode, two backslashes */
\end{alltt}
 \textbf{Implementation minimum:} Implementations should support
literal strings of at least 100 characters.
(But note that the length of string expression results, \&, should
...
(0-9), or the characters underscore, dollar, and euro
\footnote{
...
sequence), or it may be a  \emph{hexadecimal or binary (see page \pageref{refhexbin}) 
numeric symbol}:ea., or it may be a number expressed in \emph{exponential
notation}.
...
The sequences "\textbf{++}"
and "\textbf{\textbackslash \textbackslash }" are not valid in NetRexx
programs.
...
'REPEAT' B+3;
\end{alltt}
\subsection{Implied semicolons and continuations}\label{"id"}
...
\item Semicolons are added automatically by NetRexx after certain
instruction keywords when in the correct context.
The keywords that may have this effect are \texttt{else},
\texttt{finally}, \texttt{otherwise}, \texttt{then}; they become
complete clauses in their own right when this occurs.
These special cases reduce program entry errors significantly.
\end{enumerate}
\subsection{The case of names and symbols}\label{"id"}
\index{Case,of names}
...
In general, NetRexx is a \emph{case-insensitive} language.
...
\subsection{Hexadecimal and binary numeric symbols}\label{"id"}
