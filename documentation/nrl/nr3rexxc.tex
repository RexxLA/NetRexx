...
Methods of the \keyword{RexxStream}, \keyword{RexxTime} and
...
\begin{enumerate}
\item All methods work on a \nr{} string of type \keyword{R\textsc{exx}}; this
...
\end{lstlisting}
 ...
 See also page \pageref{c2classic} for compatibility with the Classic
 Rexx built-in function.
...
unless the third argument is zero, see page \pageref{refrexxstream} for Class \keyword{RexxStream} information.

\section{lineout(name,string)} returns '1' or '0', indicating whether the second argument has been successfully written to the stream named by the first argument. A result of '1' means an unsuccessful write, see page \pageref{refrexxstream} for Class \keyword{RexxStream} information.
...
\section{stream(name, [operation{]}, [stream\_command{]})} (Operations)
  returns a description of the state of, or the result of an operation upon, the stream named by the first argument, see page \pageref{refrexxstream} for Class \keyword{RexxStream} information.
