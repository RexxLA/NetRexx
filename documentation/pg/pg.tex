...
the EXEC roots are the explanation why some people refer to an \nr
...
\emph{\nr Quick Beginning Guide}, we can run it with
\begin{verbatim}
    nrc -exec hello
\end{verbatim}
...
better handled by the \textbf{loop} statement. This always reminds me
about an anecdote that Andy Hertzfield tells\footnote{\url{http://www.folklore.org}}:
\begin{quotation}
...
\nr is case independent\footnote{With the default of \texttt{options
  nostrictcase} in effect.}; Java is case dependent. \nr generates the Java code with the case used in the class and method instructions. For example, if you named your class Spider in the \nr source file, the resulting Java class file is Spider.class.
...
This particular sample has been designed to do something that is hard
...
programs. Indeed, with \nr performing so much better than the closes
...
While Apple is still a closed model that allows programming only by
...
\item The prerequisites are a Java VM (JamVM installs a VM and
  ...
  \emph{Jazelle}, a specials instructionset to accellerate Java bytecode. 
\end{enumerate}
...
\texttt{jzos} have so many drawbacks that it really only is the only
