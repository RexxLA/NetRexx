...
\marginnote{\color{gray}4.03} Stream I/O was one of the casualties of an IBM Source Code Freeze around the \Rexx 4.00 era. This meant that TSO and VM/CMS \Rexx{} did not receive the code, and had to settle for EXECIO\index{EXECIO}, originally a VM utility program, which was adapted to handle \Rexx{} stems. Regina and Open Object Rexx (ooRexx) do have Stream I/O, while several dialects have idiosyncratic I/O implementations, some based on the C standard library. For a long time, MVS/TSO had a separately installable stream I/O library; not many IBM mainframe sites did install it, and consequently it could not be counted upon to be available. These functions are, however, available on z/OS USS (Unix Systems Services).
...
For \keyword{char()} and \keyword{charin()} the same rules apply, but they work on one (Unicode) character at a time. Because of this, they are not as fast, as they cannot optimally used a buffered input mechanism.
